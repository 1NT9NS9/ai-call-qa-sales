...
class Stage4RetryAndReviewVerificationTests(unittest.TestCase):
    def _run_analysis(
        self,
        *,
        transcript_segments,
    ...
    def test_stage4_invalid_json_retries_once_and_stops(self) -> None:
        (
            result,
            error,
            persisted_analysis,
        ...
        self.assertEqual(
            ...
            2,
            "expected invalid JSON to trigger exactly one retry and not retry a second time",
        )
        ...
        self.assertFalse(
            ...
            "expected the retry-success path not to route through review mode",
        )
        self.assertEqual(
    ...
    def test_stage4_schema_invalid_json_retries_once_and_stops(self) -> None:
        (
            result,
            error,
            persisted_analysis,
        ...
        self.assertEqual(
            len(fake_chat_model.bound_model.invocations),
            2,
            "expected schema-invalid JSON to trigger exactly one retry and not retry a second time",
        )
        self.assertIsNotNone(
            persisted_analysis,
            "expected the schema-invalid retry-success path to persist a CallAnalysis record",
        )
        self.assertFalse(
            persisted_analysis.review_required,
            "expected the schema-invalid retry-success path not to route through review mode",
        )
        self.assertEqual(
            persisted_call.processing_status,
            persistence_models.CallProcessingStatus.ANALYZED,
    ...
    def test_stage4_repeated_invalid_output_persists_review_required_state(self) -> None:
        (
            result,
            error,
            persisted_analysis,
        ...
        self.assertEqual(
            result["needs_review"],
            True,
            "expected repeated invalid output to return a review receipt",
        )
        self.assertIsNotNone(
            persisted_analysis,
        ...
        self.assertTrue(
            persisted_analysis.review_required,
            "expected repeated invalid output to persist review_required = true",
        )
        self.assertTrue(
            persisted_analysis.review_reasons,
            "expected repeated invalid output to persist populated review reasons",
        )
        self.assertEqual(
            persisted_call.processing_status,
            persistence_models.CallProcessingStatus.ANALYZED,
            "expected repeated invalid output review routing to keep the call in analyzed",
        )
    ...
    def test_stage4_low_confidence_output_persists_review_required_state(self) -> None:
        ...
        self.assertEqual(
            result["needs_review"],
            True,
            "expected low-confidence output to route through review mode",
        )
        self.assertIsNotNone(
            persisted_analysis,
        ...
        self.assertTrue(
            persisted_analysis.review_required,
            "expected low-confidence output to persist review_required = true",
        )
        self.assertTrue(
            persisted_analysis.review_reasons,
            "expected low-confidence output to persist populated review reasons",
        )
        self.assertEqual(
            persisted_call.processing_status,
            persistence_models.CallProcessingStatus.ANALYZED,
    ...
    def test_stage4_empty_or_too_short_transcript_moves_call_to_failed(self) -> None:
        (
            _result,
            error,
            persisted_analysis,
            ...
            responses=[json.dumps(VALID_ANALYSIS_RESULT)],
        )

        self.assertIsInstance(
            error,
            RuntimeError,
            "expected an empty or too-short transcript to follow the Stage 4 hard-fail rule",
        )
        self.assertEqual(
            str(error),
            analysis_service_module.TRANSCRIPT_TOO_SHORT_ERROR,
            "expected the hard-fail branch to use the Stage 4 transcript error message",
        )
        self.assertIsNone(
            persisted_analysis,
        ...
        self.assertEqual(
            len(fake_chat_model.bound_model.invocations),
            0,
            "expected the transcript hard-fail branch not to invoke the model",
        )
